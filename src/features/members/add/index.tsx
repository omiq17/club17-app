...
export function AddMemberModal(props: IProps) {
  ...
  const onAddMember = async (e) => {
    ...
    if (validation.isValid) {
      ...
      // Upload avatar through UploadCare API
      let formData = new FormData();
      formData.append('UPLOADCARE_PUB_KEY', 'dccae71ba51a4315daa6');
      ...
      await axios({
        ...
        .then(async (result: AxiosResponse<{ file: string }>) => {
          ...
          const loginAction = await dispatch(addMember({ ...data, avatar: result.data.file, token }))
          if (addMember.fulfilled.match(loginAction)) {
            onClose()
            setData(undefined)
          }
        })
